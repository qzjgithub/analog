 ...
 */
const generateMessage = (option,operate,level)=>{
  ...
  let parr = [];
  interfacesId && parr.push(dbinterfaces.getInterfacesById(projectAccount,{ id: interfacesId }));
  modularId && parr.push(dbmodular.getModularById(projectAccount, { id: modularId }));
  projectAccount && parr.push(dbproject.getProjectByAccount(projectAccount));
  userAccount && parr.push(dbuser.getUserByAccount({ account: userAccount }));
  return new Promise((resolve,reject)=>{
    Promise.all(arr).then((datas)=>{
      interfaces = datas[0].length ? {} : datas[0][0];
      modular = datas[1].length ? {} : datas[1][0];
      project = datas[2].length ? {} : datas[2][0];
      user = datas[3].length ? {} : datas[3][0];
      ...
      let date = new Date();
      dbmessage.addMessage(Object.assign(arguments[0],{ content: content ,createdTime: date }))
        .then(() => dbmessage.getMessageByCreatedTime(date))
        .then((message) => {
          if(message.length){
            message = message[0];
            resolve(message);
...
const contactMessageUser = ( message ) =>{
  this.getFollowUser( message )
    .then((relations) => {
      relations = Array.from(new Set(relations));
      this.addMessageUser(message['id'],relations,0,null);
    })
    .catch((err) => {
      console.log(err);
    })
...
const getFollowUser = ( option,level) =>{
  ...
  return new Promise((resolve,reject)=>{
    let relations = [];
    if(level>=1){
      dbmessage.getUserRelateion( projectAccount, { type: 'project', relatedId: ''} )
        .then((pr)=>{
          relations = [...relations,...pr];
          if(level>=2){
            this.getModularFollow( projectAccount, modularId, null, null )
              .then((mr) => {
                relations = [...relations,...mr];
                if(level>=3){
                  dbmessage.getUserRelation( projectAccount, { type: 'interfaces', relatedId: interfacesId })
                    .then((ir) => {
...
const getModularFollow = ( projectAccount, modularId, relations, theres ) => {
  return new Promise(( resolve, reject ) => {
    ...
    relations = relations || [];
    dbmessage.getUserRelation( projectAccount, { type: 'modular', relatedId: modularId })
      .then((data)=>{
        relations = [ ...relations,...data ];
        dbmodular.getModularById(projectAccount, { id: modularId })
          .then((modular)=>{
            modular = modular.length ? {} : modular[0];
            if(modular.parent){
              this.getModularFollow( projectAccount, modular.parent, relations, theres);
            }else{
              theres(relations);
            }
          })
...
const addMessageUser = ( messageId ,relations, index, theres) => {
  return new Promise(( resolve ,reject ) => {
    ...
    if(index >= relations.length ){
      ...
      dbmessage.addMessage( { recipient: relations[index], messageId } )
        .then(() => {
          index++;
          this.addMessageUser( messageId, relations, index, theres);
        })
        .catch((err)=>{
          theres(err);
        })
